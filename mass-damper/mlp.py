...
class PINN:
    ...
    def forward(self, params, t, p_vec):
        """
        MLP forward pass that supports BOTH:
        - params = [(W,b), (W,b), ...]
        - params = FrozenDict({'Dense_0': {'kernel':..., 'bias':...}, ...})
        """
        if isinstance(params, (dict, FrozenDict)):
            params_list = []
            for layer in params.values():                      # layer = {'kernel', 'bias'}
                W = layer["kernel"]
                b = layer["bias"]
        ...
        inputs = jnp.concatenate([t[None], p_vec])
        x = inputs
