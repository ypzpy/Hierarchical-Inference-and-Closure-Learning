...
def bilevel_train_step(params_alpha, params_beta, opt_state_alpha, opt_state_beta, 
    ...
    def inner_loop_fn(p_alpha, p_beta, opt_state_beta, rng_key):
        loss_and_grad_beta = jax.value_and_grad(fno_physics_loss, argnums=0)
        ...
        (p_beta, opt_state_beta), inner_losses = lax.scan(
            loop_body, (p_beta, opt_state_beta), jnp.arange(cfg.models.fno_beta.fno_steps)
        )
...
def main(cfg):
    ...
    fno_model = FNO(cfg, FNO_utils1D)
    params_beta, opt_state_beta = fno_model.init_model(fno_key, jnp.ones((T-1,4)))
    beta_update_fn = fno_model.update
    beta_apply_fn = fno_model.vmap_z_call
    ...
    for epoch in range(10000):
        ...
        end_time = time.time()
        print(f"Epoch {epoch} completed in {end_time - start_time:.6f} seconds.")
        ...
        if epoch % 100 == 0 and epoch > 0:
            ...
            inputs = jnp.tile(parameter_matrix[:, None, :], (1, T-1, 1))
            ...
            out = fno_model.vmap_z_call(params_beta, inputs)
            initial_conditions = jnp.concatenate([x0, v0], axis=1)[:, None, :]
            out_fno = jnp.concatenate([initial_conditions, out], axis=1)
            ...
            axs_traj_0[0].plot(out_fno[0, :, 0], label='Predicted')
            axs_traj_0[0].set_title("Position (System 0)")
            axs_traj_0[0].legend()
            axs_traj_0[1].plot(velocity_path[0][:], label='True')
            axs_traj_0[1].plot(out_fno[0, :, 1], label='Predicted')
            ...
            # Plot for System 11
            fig_traj_11, axs_traj_11 = plt.subplots(1, 2, figsize=(12, 5))
            axs_traj_11[0].plot(simulations[11][:], label='True')
            axs_traj_11[0].plot(out_fno[11, :, 0], label='Predicted')
            axs_traj_11[0].set_title("Position (System 11)")
            axs_traj_11[0].legend()
            axs_traj_11[1].plot(velocity_path[11][:], label='True')
            axs_traj_11[1].plot(out_fno[11, :, 1], label='Predicted')
            axs_traj_11[1].set_title('Velocity (System 11)')
            axs_traj_11[1].legend()
            fig_traj_11.tight_layout()
            log_data["Plots/Trajectory Comparison (System 11)"] = wandb.Image(fig_traj_11)
            plt.close(fig_traj_11)
...
if __name__ == "__main__":
    ...
    cfg = load_config(path='config_FNO_physics.yml')
    main(cfg)
