...
def bilevel_train_step(params_alpha, params_beta, opt_state_alpha, opt_state_beta, 
    ...
    def inner_loop_fn(p_alpha, p_beta, opt_state_beta, rng_key):
        ...
        sampled_theta = systems_params[sys_indices, :].reshape(-1,3)

        def loop_body(carry, i):
            p_beta, opt_state_beta, fixed_key = carry
            ...
            return (p_beta, opt_state_beta, fixed_key), inner_loss_val

        (p_beta, opt_state_beta, fixed_key), inner_losses = lax.scan(
            loop_body, (p_beta, opt_state_beta, fixed_key), jnp.arange(cfg.models.pinn_beta.fno_steps)
        )
...
def main(cfg):
    ...
    for epoch in range(10000):
        ...
        if epoch % 100 == 0 and epoch > 0:
            ...
            axs_traj_0[0].plot(simulations[3][:], label='True')
            axs_traj_0[0].plot(out_pinn[3, :, 0], label='Predicted')
            axs_traj_0[0].set_title("Position (System 3)")
            axs_traj_0[0].legend()
            axs_traj_0[1].plot(simulations[9][:], label='True')
            axs_traj_0[1].plot(out_pinn[9, :, 0], label='Predicted')
            axs_traj_0[1].set_title('Velocity (System 9)')
            axs_traj_0[1].legend()
