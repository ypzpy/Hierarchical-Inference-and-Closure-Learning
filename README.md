...
At each epoch, an **ensemble MALA** sweep first updates the hierarchical posterior over per-system parameters $\{\theta_i\}$ and population-level hyperparameters $\phi = (\mu_\phi, \tau_\phi)$, using the current $\alpha$ and $\beta$. The resulting chain samples are then passed to a **bilevel optimization** step:
