...
def bilevel_train_step(params_alpha, params_beta, opt_state_alpha, opt_state_beta, 
    ...
    def inner_loop_fn(p_alpha, p_beta, opt_state_beta, rng_key):
        ...
        # rng_key, subkey = jax.random.split(rng_key)
        # idx = jax.random.choice(subkey,a=20,shape=(2,),replace=False)
        # sampled_theta = sampled_theta[idx, :]

        def loop_body(carry, i):
            p_beta, opt_state_beta, rng_key, fixed_key = carry
            
            rng_key, subkey = jax.random.split(rng_key)
            idx = jax.random.choice(subkey,a=20,shape=(1,),replace=False)
            # sampled_theta = sampled_theta[idx, :]
            
            inner_loss_val, grads_beta = loss_and_grad_beta(
                p_beta, p_alpha, sampled_theta[idx, :], alpha_apply_fn, beta_apply_fn, rng_key=fixed_key
            )
            ...
            return (p_beta, opt_state_beta, rng_key, fixed_key), inner_loss_val

        rng_key, subkey, fixed_key = jax.random.split(rng_key,3)
        (p_beta, opt_state_beta, subkey, fixed_key), inner_losses = lax.scan(
            loop_body, (p_beta, opt_state_beta, subkey, fixed_key), jnp.arange(cfg.models.pinn_beta.fno_steps)
        )
...
def single_chain_initialisation(rng_key, cfg):
    ...
    z1_key, z2_key, z3_key = random.split(theta_key, 3)
    z1_params = cfg.prior.mean.z1_mean + random.normal(z1_key, (cfg.data_systems.n_systems,)) * jnp.sqrt(cfg.prior.variance.z1_variance)
    z2_params = cfg.prior.mean.z2_mean + random.normal(z2_key, (cfg.data_systems.n_systems,)) * jnp.sqrt(cfg.prior.variance.z2_variance)
    z3_params = cfg.prior.mean.z3_mean + random.normal(z3_key, (cfg.data_systems.n_systems,)) * jnp.sqrt(cfg.prior.variance.z3_variance)

    theta = jnp.stack([z1_params, z2_params, z3_params], axis=-1)
...
def main(cfg):
    ...
    key, rng_key = jax.random.split(rng_key,2)
    batched_parameters = vmap_single_chain_initialisation(key, cfg).reshape(cfg.langevin_sampler.n_chains,-1)
    print("Shape: batched_parameters ", batched_parameters.shape)
    ...
    for epoch in range(20000):
        ...
        log_data = {
            ...
            "Hyperprior/z1_0": batched_parameters[0][0],
            "Hyperprior/z2_0": batched_parameters[0][1],
             "Hyperprior/z3_0": batched_parameters[0][2],
            "Hyperprior/z1_1": jnp.exp(batched_parameters[0][3]),
            "Hyperprior/z2_1": jnp.exp(batched_parameters[0][4]),
             "Hyperprior/z3_1": jnp.exp(batched_parameters[0][5]),
        }
        ...
        if epoch % 50 == 0 and epoch > 0:
            ...
            y_pred_plot = alpha_apply_fn(params_alpha,x_plot)
            ...
            pred = out[..., None].reshape(parameter_matrix.shape[0], nx, ny)
        ...
        print("      z1, z2, z3 = ", mean_of_chain[:3])
